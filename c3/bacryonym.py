...
def get_args():
	...
	parser.add_argument('-n', '--num', help='Maximum number of definitions', 						type=int, metavar='NUM', default=5)
	parser.add_argument('-w', '--wordlist', help='Dictionary/word file',
						type=str, metavar='STR', default='/usr/share')
...
def main():
	...
	for word in open(wordlist).read().split():
		...
		if re.match(goodword, clean) and clean not in ssn and clean not in badwords:
			seen[clean] = 1
